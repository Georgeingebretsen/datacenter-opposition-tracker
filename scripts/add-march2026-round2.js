...
const research = JSON.parse(fs.readFileSync(path.join(dataDir, 'new-fights-march2026-round2.json'), 'utf8'));
...
for (const entry of newEntries) {
  ...
  const similar = fights.find(f =>
    f.state === entry.state &&
    f.jurisdiction.toLowerCase().includes(entry.jurisdiction.toLowerCase().split(' ')[0])
  );
  if (similar) {
